# start6 but enhanced using llms for comments and better code
# 
...
class Trader:
    def __init__(self):
        ...
        self.size_scale = 10  # how aggressively size increases with |z|
        self.mm_spread = 2   # market-making spread
    ...
    def run(self, state, current_position):
        ...
        # --- Dynamically scaled order size ---
        dynamic_size = int(self.base_order_size + self.size_scale * abs(zscore))
        dynamic_size = min(dynamic_size, self.position_limit)

        # --- Entry logic ---
        if zscore < -self.z_entry_threshold and vol_spike and current_position < self.position_limit:
            qty = min(dynamic_size, self.position_limit - current_position)
        ...
        # --- Exit logic ---
            ...
            qty = min(dynamic_size, abs(current_position))
        ...
        # --- Market Making Fallback ---
        if not traded and abs(current_position) < self.position_limit:
            mm_bid = mid_price - self.mm_spread
            mm_ask = mid_price + self.mm_spread
            orders.append(Order("PRODUCT", mm_bid, self.base_order_size))
            orders.append(Order("PRODUCT", mm_ask, -self.base_order_size))

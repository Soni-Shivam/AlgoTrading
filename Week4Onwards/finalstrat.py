...
class JolteonBBRSIStrategy(BaseClass):
    def __init__(self):
    ...
    def get_orders(self, state, orderbook: OrderBook, position: int):
        ...
        mean_price = np.mean(self.prices)
        std_price = np.std(self.prices)
        upper_bb = mean_price + self.bb_multiplier * std_price
        lower_bb = mean_price - self.bb_multiplier * std_price
        rsi = self.compute_rsi(self.prices)

        qty = min(self.base_qty, self.max_position)

        # Entry Long
        if mid_price < lower_bb and rsi < self.rsi_oversold and position < self.max_position:
            orders.append(Order(self.product_name, best_bid + 1, min(qty, self.max_position - position)))

        # Entry Short
        elif mid_price > upper_bb and rsi > self.rsi_overbought and position > -self.max_position:
            orders.append(Order(self.product_name, best_ask - 1, -min(qty, self.max_position + position)))

        # Exit Neutral RSI Zone
        ...
        # Stop Loss (price deviates too far from BB)
        elif position > 0 and mid_price < lower_bb - self.stop_loss_multiplier * std_price:
            orders.append(Order(self.product_name, best_bid, -abs(position)))
        elif position < 0 and mid_price > upper_bb + self.stop_loss_multiplier * std_price:
            orders.append(Order(self.product_name, best_ask, abs(position)))

        # Market Making (only if no active signal and flat or small position)
        if not orders and abs(position) < self.max_position:
            orders.append(Order(self.product_name, mid_price - self.mm_spread, self.base_qty))
            orders.append(Order(self.product_name, mid_price + self.mm_spread, -self.base_qty))
